	...
	*/
	class WebBase extends DBM {
		...
		function check_session($session){
			file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." CHECK SESSION 1 ] : ".$session."\r\n\r\n", FILE_APPEND);
			if (strlen($session) == 0){
				file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." CHECK SESSION 2 ] : \r\n\r\n", FILE_APPEND);
				$result['success'] = false;
				$result['msg'] = 'session不能为空';
			}else{
				// 匹配session
				if ($this->is_session_exsit($session)){
					file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." CHECK SESSION 3 ] : \r\n\r\n", FILE_APPEND);
					// 获取未过期的session对应信息
					$res = $this->match_session($session);
					if ($res){
						file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." CHECK SESSION 4 ] : \r\n\r\n", FILE_APPEND);
						...
						file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." CHECK SESSION 5 ] : \r\n\r\n", FILE_APPEND);
						$result['success'] = false;
						$result['msg'] = 'session已失效';
					}
				}else{
					file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." CHECK SESSION 6 ] : \r\n\r\n", FILE_APPEND);
					$result['success'] = false;
