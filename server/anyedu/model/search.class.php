	...
	*/
	class Search_Model extends WebBase {
		...
		function view(){
			...
			file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." VIEW RECORDS 1 ] : \r\n\r\n", FILE_APPEND);
			
			$POST = json_decode(file_get_contents("php://input"), true);
			
			file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." VIEW RECORDS 1 AFTER  ] : SESSION :".$POST['session'].", INDEX: ".$POST['index'].",NUMBER:".$POST['number']." \r\n\r\n", FILE_APPEND);
			...
			if (isset($POST['session']) 
				...
				// 验证身份
				file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." VIEW RECORDS 2 ] : \r\n\r\n", FILE_APPEND);
			
				$result = $this->check_session($session);
				if ($result['success']){
					file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." VIEW RECORDS 3 ] : \r\n\r\n", FILE_APPEND);
			
					$result = array();
					$sql = "SELECT `id`, `duration`, `tag`, `src`, `post_datetime` FROM `t_whisper` WHERE `is_valid` is true AND `poster_openid` = '".$this->openid()."' ORDER BY `post_datetime` DESC LIMIT ".$index.",".$number;
					$t = $this->query($sql);
					if ($t !== false){
						file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." VIEW RECORDS 4 ] : \r\n\r\n", FILE_APPEND);
						...
						file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." VIEW RECORDS 5 ] : \r\n\r\n", FILE_APPEND);
				...
				file_put_contents("/www/api/log", "[".date("Y/m/d H:i:s")." VIEW RECORDS 6 ] : \r\n\r\n", FILE_APPEND);
